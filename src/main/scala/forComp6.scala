...
object forComp6 {
  ...
  def compute(x: Int, y: Int): SResult[Throwable, Int] = {
    for {
      a <- if (x > 0) SRight(x * 2) else SLeft(new IllegalArgumentException("x must be positive"))
      b <- if (y != 0) SRight(y + 10) else SLeft(new ArithmeticException("y cannot be zero"))
      result <- SRight(a / y)
    } yield result
  }
}
